...
@pytest.mark.parametrize(("status_code", "output", "exception"),
                         [(200, True, None),
                          (404, False, None),
                          (200, False, requests.ConnectionError)])
def test_client_healthcheck(starpack_client, requests_mock, status_code: int, output: bool,
...
@pytest.mark.parametrize(("status_code", "output"), [(200, "Successfully deployed"),
                                                     (500, "500"),
                                                     (300, "300")])
def test_client_deploy(starpack_client, requests_mock, capsys, status_code: int, output: str):
...
@pytest.mark.parametrize(("status_code", "output"), [(200, "Successfully packaged"),
                                                     (500, "500"),
                                                     (300, "300")])
def test_client_package(starpack_client, requests_mock, capsys, status_code: int, output: str):
